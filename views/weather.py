...
class WeatherView(BaseView):
    ...
    def _render_condition(self, condition: WeatherCondition, x_pos: int):
        if condition != self._condition:
            ...
            self._icon = Image.open(
                get_abs_file_path(f"assets/{icon_data['name']}.ppm")
            ).convert("RGB")
        ...
        self._offscreen_canvas.SetImage(
            self._icon, x_pos + self._icon_x_offset, self._icon_y_offset, unsafe=False
        )
    ...
    def _render(self):
        ...
        x_pos = (
            get_x_pos_for_centered_text_and_obj(
                center_pos=16,
                text=temperature,
                font_width=size["width"],
                obj_width=ICON_WIDTH,
            )
            - 1
        )
